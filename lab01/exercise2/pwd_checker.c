#include <string.h>
#include <assert.h> // AS WE ARE CALLING ASSERT FUNCTION IN OUR CHECK_PASSWORD FUNCTION, WE HAVE TO INCLUDE ITS HEADER
...
bool check_password(const char *first_name, const char *last_name, const char *password) {
    ...
    // ADDED assert(length/name /...) for first test case but it gives errors for next test cases so we have to remove it 
